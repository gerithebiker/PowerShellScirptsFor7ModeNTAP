...
.NOTES
  ...
  Change history: 	1.0 (Geri) 08/20/2019
					...
					1.3.2 (Geri) 10/23/2019
						Unfortunately Molina environment is pretty much NOT standard, I cannot use the mount point name to select the disks. 
							(There can be the same name folders in different dives, like G:\MP_DB1 and F:\MP_DB1, so I have to use the full path)
						For the off-shore colleagues the "onedrive" variable is not set, I had to change the csv output path to the desktop
					...
					1.3.5 (Geri) 02/17/2020
						Molina's standard is that the drive mount point should have the drive letter in it, like H_MP_AGMI_DB1.
						There are mountpoints not containing the drive letter, only something like MPDB_LOG1 and the same mount folder can exist under other mountpoints.
...
.EXAMPLE
  Get-MountPointData.ps1 -Computers DC01OLTPDBPP026,DC01OLTPDBPP023 -SelectDrives H:\H_MP_AGMI
...
.EXAMPLE
  Get-MountPointData.ps1 -Computers DC01OLTPDBPP026,DC01OLTPDBPP023 -SelectDrives H:\H_MP_AGMI_DB1,H:\H_MP_AGMI_DB2,H:\H_MP_AGMI_DB3 -Progress
...
############################################
	...
	# Currently the biggest allowed dirve size at Molina is 4T. If it changes, replace the size here
	$MaxLUNSize = 4096
...
# Hastable for hosts
$AdminHosts = @{ 	"000197200503" = "vb6empsv02"
					"000197500124" = "vb102emspv01"
					"000197500126" = "vb103emspv01"
					"000197500127" = "vb202emspv01"
				}
